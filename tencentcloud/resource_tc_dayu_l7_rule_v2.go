...
func resourceTencentCloudDayuL7RuleCreateV2(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := dayuService.CreateL7RuleV2(ctx, resourceType, resourceId, resourceIp, rule)
		if e != nil {
			return retryError(e)
		}
		ruleId = result
	...
	readyFlag, rErr := checkL7RuleStatusV2(meta, resourceType, resourceId, ruleId, "create")
	if rErr != nil {
		return rErr
	}
	if !readyFlag {
...
func resourceTencentCloudDayuL7RuleUpdateV2(d *schema.ResourceData, meta interface{}) error {
	...
	if ruleFlag {
		...
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			e := dayuService.ModifyL7RuleV2(ctx, resourceType, resourceId, rule)
			if e != nil {
				return retryError(e)
			}
			return nil
...
func checkL7RuleStatusV2(meta interface{}, resourceType string, resourceId string, ruleId string, checkType string) (status bool, errRrt error) {
	...
	err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		sRule, health, has, err := dayuService.DescribeL7RuleV2(ctx, resourceType, resourceId, ruleId)
		if err != nil {
			return retryError(err)
		}
